...
router.post('/update',upload.fields(type), async function(req,res){
    ...
    if(typeof(message) === typeof({})){
        ...
        if(req.body.content === undefined || req.body.title === undefined || req.body.post_id === undefined){
            res.status(200).json({
                "status": "400",
                            ...
                            connection.query(`Select * from ${dbconfig.database}.group where group_id ="${rows[0].group_id}" and manager="${message.id}"`, function(err,rows,fields){
                                if(err){
                                    ...
                                    if(rows[0]===undefined){
                                        ...
                                        connection.query(`Update ${dbconfig.database}.post Set title = "${post.title}", content = "${post.content}" where post_id = "${post.post_id}"`,  function(err,rows,fields){
                                            if(err){
                                                res.status(200).json({
                                                    "status": "400",
                                                    "message": err
